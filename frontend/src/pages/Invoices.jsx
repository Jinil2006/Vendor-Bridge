...
import { Search, Download, Printer, Mail } from 'lucide-react';
...
const Invoices = () => {
      ...
      <div className="po-page" style={{ padding: '1rem', animation: 'fadeIn 0.4s ease-out' }}>
        <div className="page-header" style={{ marginBottom: '2rem' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-muted)' }}>#{selectedInvoice.poId}</p>
          </div>
          <button className="btn btn-secondary" onClick={() => setIsViewing(false)} style={{ border: '1px solid var(--border-color)', background: 'transparent' }}>Back to Invoices</button>
        </div>
        ...
        <div style={{ background: '#fff', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '3rem', maxWidth: '900px', margin: '0 auto', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3rem', borderBottom: '2px solid var(--border-color)', paddingBottom: '2rem' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ marginBottom: '1rem' }}>
                 ...
                 <span style={{ fontWeight: '700', fontSize: '1.1rem' }}>{selectedInvoice.poId}</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'auto auto', gap: '0.5rem 1rem', textAlign: 'right', fontSize: '0.9rem' }}>
                <span style={{ color: 'var(--text-muted)' }}>PO Date:</span><span>{selectedInvoice.date}</span>
                <span style={{ color: 'var(--text-muted)' }}>Invoice Date:</span><span>{selectedInvoice.invoiceDate}</span>
                <span style={{ color: 'var(--text-muted)' }}>Due Date:</span><span style={{ color: 'var(--danger)', fontWeight: '600' }}>{selectedInvoice.dueDate}</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ marginBottom: '3rem' }}>
            ...
            <p style={{ fontWeight: '600', fontSize: '1.1rem', marginBottom: '0.25rem' }}>{selectedInvoice.vendor.name}</p>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: '1.6' }}>{selectedInvoice.vendor.address}<br/>GST: {selectedInvoice.vendor.gst}</p>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '3rem' }}>
            <div style={{ width: '350px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 1rem', color: 'var(--text-muted)' }}>
                <span>CGST (9%)</span><span>${selectedInvoice.cgst.toLocaleString()}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 1rem', color: 'var(--text-muted)' }}>
                <span>SGST (9%)</span><span>${selectedInvoice.sgst.toLocaleString()}</span>
              </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border-color)', paddingTop: '2rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
               ...
               <span style={{ padding: '0.5rem 1rem', background: selectedInvoice.status === 'Overdue' ? 'rgba(239, 68, 68, 0.1)' : (selectedInvoice.status === 'Paid' ? 'rgba(34, 197, 94, 0.1)' : 'rgba(234, 179, 8, 0.1)'), color: selectedInvoice.status === 'Overdue' ? 'var(--danger)' : (selectedInvoice.status === 'Paid' ? 'var(--success)' : '#ca8a04'), borderRadius: '4px', fontWeight: '600', fontSize: '0.85rem' }}>{selectedInvoice.status}</span>
  ...
  return (
    <div className="invoices-page" style={{ padding: '1rem', animation: 'fadeIn 0.4s ease-out' }}>
      <div className="page-header">
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: '800' }}>Invoices</h1>
          <p style={{ color: 'var(--text-muted)' }}>Manage your incoming vendor invoices.</p>
        </div>
      </div>
      ...
      <div className="card table-card" style={{ background: '#fff', borderRadius: '12px', padding: '1.5rem', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
        <div className="table-toolbar" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
          <div className="search-box" style={{ display: 'flex', alignItems: 'center', background: 'var(--background-light)', padding: '0.5rem 1rem', borderRadius: '8px', width: '300px' }}>
            ...
            <input type="text" placeholder="Search Invoices..." style={{ border: 'none', background: 'transparent', outline: 'none', width: '100%' }} />
          </div>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
              <th style={{ padding: '1rem', color: 'var(--text-muted)', fontWeight: '600' }}>Invoice#</th>
              ...
              <th style={{ padding: '1rem', color: 'var(--text-muted)', fontWeight: '600' }}>Due Date</th>
          ...
          <tbody>
            {invoices.map((inv) => (
              <tr key={inv.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                <td style={{ padding: '1rem', fontWeight: '600' }}>{inv.id}</td>
                <td style={{ padding: '1rem' }}>{inv.poId}</td>
                <td style={{ padding: '1rem' }}>{inv.vendor.name}</td>
                <td style={{ padding: '1rem', color: inv.status === 'Overdue' ? 'var(--danger)' : 'inherit' }}>{inv.dueDate}</td>
                <td style={{ padding: '1rem', fontWeight: '600' }}>${inv.grandTotal.toLocaleString()}</td>
                <td style={{ padding: '1rem' }}>
                  <span style={{ padding: '0.4rem 0.8rem', background: inv.status === 'Overdue' ? 'rgba(239, 68, 68, 0.1)' : 'rgba(234, 179, 8, 0.1)', color: inv.status === 'Overdue' ? 'var(--danger)' : '#ca8a04', borderRadius: '4px', fontSize: '0.85rem', fontWeight: '600' }}>{inv.status}</span>
                </td>
                <td style={{ padding: '1rem' }}>
                  <button className="btn btn-secondary" style={{ padding: '0.4rem 1rem', fontSize: '0.85rem', background: 'transparent', border: '1px solid var(--border-color)' }} onClick={() => { setSelectedInvoice(inv); setIsViewing(true); }}>View</button>
                </td>
              </tr>
            ))}
          </tbody>
